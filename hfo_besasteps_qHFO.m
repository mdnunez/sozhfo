function [fail,subject,abesapath] = hfo_besasteps_qHFO(varargin)
%HFO_BESASTEPS_QHFO - Script that follows the steps to extract FALSE HFOs from BESA files for artifact correction
%
...
%   ====        =================            =====================
%  09/04/20      Michael Nunez               Converted from hfo_besasteps.m
%  09/22/20      Michael Nunez      Correction to deal with no pop detection artifact
%  10/27/20      Michael Nunez            Correction to deal with empty false_candidate findings
%  11/09/20      Michael Nunez            Create .csv file even if no false_candidates exist
...
% if canloadbesa & (besatrack == 22), %Temporary code for debugging
if canloadbesa,
	abesapath = fullpath;
	wheresub = regexp(besas(bnum).name,'Patient');
	subject = besas(bnum).name((wheresub(1)):(wheresub(1)+3));
	...
	while currentsamp < header.nSamples - buffersamps
		...
		cd(saveloc);
		hfocsvloc = sprintf('%s_falseHFOdata%d.csv',subject,besatrack);
