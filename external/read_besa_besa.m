...
% 2018 Edited by Michael D. Nunez, University of California, Irvine

%% Record of Revisions
%   Date           Programmers               Description of change
%   ====        =================            =====================
%  2016      Kristopher Anderson                   Original code
% 10/5/17      Michael Nunez           Changed bitand() to work with 
%                                             older MATLAB versions
% 03/19/18      Michael Nunez   Removed warning in read_BEVT to remove bug
...
if bitand(uint32(flag_BDAT),uint32(1))
  data_type = CONST_INT16;
else
  data_type = CONST_FLOAT;
end
if bitand(uint32(flag_BDAT),uint32(hex2dec('0010')))
...
while ftell(fid) < (uint64(channel_info.offsets(end))+uint64(channel_block_length))
  ...
  switch current_tag
    case 'CHFL'
      ...
      channel_info.channel_flags.BSA_ELECTRODE_COORDINATES_FROM_LABELS = logical(bitand(uint32(channel_info.channel_flags.flag),uint32(hex2dec('0001'))));
      channel_info.channel_flags.BSA_SUPPRESS_SPHERE_TO_ELLIPSOID_TRANSFORMATION = logical(bitand(uint32(channel_info.channel_flags.flag),uint32(hex2dec('0002'))));
      channel_info.channel_flags.BSA_ELECTRODE_COORDINATES_ON_SPHERE = logical(bitand(uint32(channel_info.channel_flags.flag),uint32(hex2dec('0004'))));
      channel_info.channel_flags.BSA_ADAPT_SPHERICAL_EEG_TO_MEG_COORDS = logical(bitand(uint32(channel_info.channel_flags.flag),uint32(hex2dec('0008'))));
      channel_info.channel_flags.BSA_SOURCE_CHANNELS_DERIVED_FROM_MEG = logical(bitand(uint32(channel_info.channel_flags.flag),uint32(hex2dec('0010'))));
    case 'CHTS'
      ...
      channel_info.channel_states(channel_n).BSA_CHANTYPE_UNDEFINED = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00000000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_POLYGRAPHIC = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00010000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_TRIGGER = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00020000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_CORTICALGRID = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00040000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_INTRACRANIAL = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00080000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_SCALPELECTRODE = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00100000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_MAGNETOMETER = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00200000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_AXIAL_GRADIOMETER = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00400000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_PLANAR_GRADIOMETER = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('01000000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_MEGREFERENCE = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00800000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_NKC_REFERENCE = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('02000000'))));
      channel_info.channel_states(channel_n).BSA_CHANTYPE_CHANSTATE_BAD = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00000001'))));
      channel_info.channel_states(channel_n).BSA_CHANSTATE_REFERENCE = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00000002'))));
      channel_info.channel_states(channel_n).BSA_CHANSTATE_INTERPOLRECORDED = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00000004'))));
      channel_info.channel_states(channel_n).BSA_CHANSTATE_INVISIBLE = logical(bitand(uint32(channel_info.channel_states(channel_n).flag),uint32(hex2dec('00001000'))));
    case 'CHCO'
    ...
    case 'CHFI'
      ...
      while ftell(fid) < offset_end_chfi
        ...
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_ACTIVE      = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000001'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_ACTIVE     = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000002'))));
        channel_info.filter_info(channel_n).state.FLT_NOTCH_ACTIVE          = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000004'))));
        channel_info.filter_info(channel_n).state.FLT_BAND_PASS_ACTIVE      = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000008'))));
        channel_info.filter_info(channel_n).state.FLT_PRE_LOWCUTOFF_ACTIVE  = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('01000000'))));
        channel_info.filter_info(channel_n).state.FLT_PRE_SUBTRACT_BASELINE = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('02000000'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_TYPE_FORWARD    = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000000'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_TYPE_ZERO_PHASE = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000010'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_TYPE_BACKWARD   = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000020'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_SLOPE_06DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000000'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_SLOPE_12DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000100'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_SLOPE_24DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000200'))));
        channel_info.filter_info(channel_n).state.FLT_LOWCUTOFF_SLOPE_48DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000300'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_TYPE_ZERO_PHASE = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000300'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_TYPE_FORWARD    = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00001000'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_TYPE_BACKWARD   = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00002000'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_SLOPE_12DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00000000'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_SLOPE_06DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00010000'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_SLOPE_24DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00020000'))));
        channel_info.filter_info(channel_n).state.FLT_HIGHCUTOFF_SLOPE_48DB = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00030000'))));
        channel_info.filter_info(channel_n).state.FLT_NOTCH_REMOVE_2ND_HARMONIC = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00100000'))));
        channel_info.filter_info(channel_n).state.FLT_NOTCH_REMOVE_3RD_HARMONIC = logical(bitand(uint32(channel_info.filter_info(channel_n).state.state),uint32(hex2dec('00200000'))));
...
while ~feof(fid) && ftell(fid) < (base_offset+base_length)
  ...
  switch current_tag
    ...
    case 'STAT'
      ...
      event_obj.state.EVT_STATE_MARKED1 = logical(bitand(uint32(event_obj.state.value),uint32(hex2dec('00000010'))));
      event_obj.state.EVT_STATE_MARKED2 = logical(bitand(uint32(event_obj.state.value),uint32(hex2dec('00000020'))));
      event_obj.state.EVT_STATE_MARKED3 = logical(bitand(uint32(event_obj.state.value),uint32(hex2dec('00000040'))));
      event_obj.state.EVT_STATE_MARKED4 = logical(bitand(uint32(event_obj.state.value),uint32(hex2dec('00000080'))));
      event_obj.state.EVT_STATE_DELETED = logical(bitand(uint32(event_obj.state.value),uint32(hex2dec('01000000'))));
    otherwise
      % Unrecognzed tag. Try to skip forward by offset
      warning('ReadBesaMatlab:WarningUnexpectedTag','Read unexpected tag [%s] at offset %d',current_tag,ftell(fid));
      if(fseek(fid,current_length,'cof') == -1)
